...
- Markup, that does not hold state or event listeners, and just adds css classes to the dom.
  E.g. `$.fn.buttonMarkup`, which is created using `<a href="..." data-role="button">`
- Markup that holds state (e.g. event listeners, ...). This markup uses the jquery ui widget factory.
  E.g. `$.mobile.button`, which is created using `<button>`.

Integration strategy:

1. We have an angular widget for data-role="page" and data-role="dialog".
   This widget will call element.page() in the compile phase.
   By this, all markup that is generated by the page plugin and
   the other jqm widgets can be used by angular for stamping (e.g. in ng-repeat),
   without calling a jqm method again, so we are fast.

2. Some jqm widgets register event handlers on elements. Those widgets
   need to be recreated every time angular stamps them (e.g. using ng-repeat).
   For this, we disable them during the call of element.page() above, and create
   angular widgets for them, which create the widget in the link phase.
   This was done for all "true" jqm widgets, i.e. those that are registered
   using $.widget.
